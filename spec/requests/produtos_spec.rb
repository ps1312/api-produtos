...
RSpec.describe 'API Produtos' do
  # dados de teste
  let!(:categoria) { create(:categoria) }
  let!(:produtos) { create_list(:produto, 20, categoria_id: categoria.id) }
  let(:categoria_id) { categoria.id }
  let(:id) { produtos.first.id }

  # Test for GET /api/v1/categorias/:categoria_id/produtos
  describe 'GET /api/v1/categorias/:categoria_id/produtos' do
    before { get "/api/v1/categorias/#{categoria_id}/produtos" }
